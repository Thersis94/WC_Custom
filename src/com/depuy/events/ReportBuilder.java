...
import com.depuy.events.PostcardSelectV2.ReqType;
import com.depuy.events.vo.AttendeeSurveyVO;
import com.depuy.events.vo.DePuyEventSeminarVO;
import com.depuy.events.vo.RsvpBreakdownVO;
import com.depuy.events.vo.report.AttendeeSurveyReportVO;
import com.depuy.events.vo.report.ComplianceReportVO;
import com.depuy.events.vo.report.CustomReportVO;
import com.depuy.events.vo.report.EventPostalLeadsReportVO;
import com.depuy.events.vo.report.LocatorReportVO;
import com.depuy.events.vo.report.PostcardSummaryReportVO;
import com.depuy.events.vo.report.RsvpBreakdownReportVO;
import com.depuy.events.vo.report.RsvpSummaryReportVO;
import com.depuy.events.vo.report.SeminarRollupReportVO;
...
public class ReportBuilder extends SBActionAdapter {
	...
	public AbstractSBReportVO generateRSVPBreakdownReport(Date start, Date end) {
		Map<String, RsvpBreakdownVO> dataMap = new HashMap<>();
		List<RsvpBreakdownVO> data = null;
		...
		try (PreparedStatement ps = dbConn.prepareStatement(sql.toString())) {
			...
			while(rs.next()) {
				...
				if (dataMap.containsKey(eventId)) {
					dataMap.get(eventId).addReferralStat(rs.getString("referral_txt"), rs.getInt("cnt"));
				} else {
					//new owner we need to capture
					dataMap.put(eventId, new RsvpBreakdownVO(rs));
				}
			}

			data = new ArrayList<>(dataMap.values());

			//lookup profiles
			ProfileManagerFactory.getInstance(getAttributes()).populateRecords(dbConn, data);
	...
	public AbstractSBReportVO generateSeminarRollupReport(Object listObj, Date start, Date end, boolean isMitek) {
		...
		List<DePuyEventSeminarVO> finalData = new ArrayList<>(data.size());
		...
		for (DePuyEventSeminarVO sem : data) {
			...
			try {
				DePuyEventSeminarVO semFull = retriever.loadOneSeminar(sem.getEventPostcardId(), actionId, ReqType.report , null, null, -1);
	...
	public AbstractSBReportVO generateRSVPSummaryReport(Object data) {
		...
		List<EventEntryVO> eventsList = new ArrayList<>();
	...
	public AbstractSBReportVO generateAttendeeSurveyReport(ActionRequest req) {
		...
		Map<String, AttendeeSurveyVO> dataMap = new LinkedHashMap<>(); //keep in the order defined by the query
		List<AttendeeSurveyVO> data = null;
		...
		if (selectAll) {
			sql.append("and c.selection_cd ~ 'EVENT_([0-9]){3,5}' "); //only ones with four trailing digits are patient seminars
		} else {
