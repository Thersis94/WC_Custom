...
/****************************************************************************
 * <b>Title</b>: CoopAdsAction.java<p/>
...
public class CoopAdsAction extends SBActionAdapter {
	...
	public void build(SMTServletRequest req) throws ActionException {
        ...
        if ("radio".equalsIgnoreCase(vo.getAdType())) return;
    	...
    	if (reqType.equals("coopAdsSurgeonApproval")) { //intercept this first since it doesn't used our statusFlg
			...
			if ("CFSEM".equalsIgnoreCase(req.getParameter("eventTypeCd"))) {
				...
				emailer.requestAdApprovalOfSurgeon(vo, site, req.getParameter("ownersEmail"), 
						req.getParameter("product"), req.getParameter("eventName"), req.getParameter("eventDt"));
			}
	...
	private CoopAdVO saveAd(SMTServletRequest req, SiteVO site, CoopAdVO vo) throws ActionException {
        ...
        if (insertRecord) {
        	sql.append("select coop_ad_id from ").append(getAttribute(Constants.CUSTOM_DB_SCHEMA));
		...
		if (insertRecord) {
			...
			if (vo.getStatusFlg() == PENDING_CLIENT_APPROVAL) sql.append(", AD_SUBMIT_DT=?, SURGEON_STATUS_FLG=? ");
		...
		vo.setSurgeonImageUrl(this.saveFile(req, "surgeonImageUrl", "oldSurgeonImageUrl"));
	 ...
	 * @param req
	 */
	private void saveAdXR(CoopAdVO vo, SMTServletRequest req) throws ActionException {
		StringBuilder sql = new StringBuilder();
    	PreparedStatement ps = null;
		...
		for (int cnt=1; StringUtil.checkVal(req.getParameter("eventEntryId_" + cnt)).length() > 0; cnt++) {
	...
	/**
	 * status update ONLY on the _COOP_AD table...used when client approves or declines the ad
	 * @param vo
	 * @param req
	...
	private void saveAdApproval(SMTServletRequest req) throws ActionException {
		StringBuilder sql = new StringBuilder();
    	PreparedStatement ps = null;
    	int statusLvl = Convert.formatInteger(req.getParameter("adStatusFlg"));
	...
	private void saveSurgeonsAdApproval(SMTServletRequest req) throws ActionException {
		StringBuilder sql = new StringBuilder();
    	PreparedStatement ps = null;
    	int statusLvl = Convert.formatInteger(req.getParameter("surgeonStatusFlg"));
	...
	private String saveFile(SMTServletRequest req, String paramNm, String oldParamNm) {
    	...
    	String fileNm = StringUtil.checkVal(req.getParameter(oldParamNm)).trim();
	...
	public void retrieve(SMTServletRequest req) throws ActionException {
		CoopAdVO vo = retrieve(req.getParameter("coopAdId"), req.getParameter("eventPostcardId"));
		req.setAttribute(RETR_ADS, vo);
	}
	
	protected CoopAdVO retrieve(String coopAdId, String postcardId) throws ActionException {
		...
		if (coopAdId.length() > 0) sql.append("or a.COOP_AD_ID=? ");
		else if (postcardId.length() > 0) sql.append("or a.EVENT_POSTCARD_ID=? ");
		...
		try {
			ps = dbConn.prepareStatement(sql.toString());
			if (coopAdId.length() > 0) ps.setString(1, coopAdId);
			else if (postcardId.length() > 0) ps.setString(1, postcardId);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				vo.setData(rs);
				vo.addEvent(rs.getString("rsvp_code_txt"), rs.getInt("order_no"));
			}
		} catch (SQLException sqle) {
			log.error("Error retrieving Co-op Ad, pkId=" + coopAdId + ", postcard=" + postcardId, sqle);
			throw new ActionException(sqle);
