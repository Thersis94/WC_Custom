...
public class CoopAdVO extends AbstractSiteBuilderVO {
	...
	private String getEventCodeByOrder(Integer order) {
		for (String key : eventCodes.keySet())
			if (eventCodes.get(key) == order) return key;
	...
	/* the optional 2nd newspaper is required if the submitter has requested addtl ads, 
	 * so we key off that for displaying info in the view.
	 */
