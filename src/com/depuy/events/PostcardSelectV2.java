...
import com.depuy.events.vo.ConsigneeVO;
//wc-depuy libs
...
public class PostcardSelectV2 extends SBActionAdapter {
	...
	private enum SurveyConst {
		...
		private String orthoQuestionGrpId;
		private String mitekQuestionGrpId;
	...
	private List<DePuyEventSeminarVO> loadSeminarList(String actionGroupId, ReqType reqType, String profileId, String sortType, int roleId) throws SQLException {
		...
		sql.append("select e.event_entry_id, e.RSVP_CODE_TXT, e.start_dt, et.type_nm, ep.event_postcard_id, ");
		...
		try (PreparedStatement ps = dbConn.prepareStatement(sql.toString())) {
			...
			while (rs.next()) {
				if (mapData.containsKey(rs.getString("event_postcard_id"))) {
					...
					vo.addJoint(Integer.toString(rs.getInt("joint_id")));
					boolean hasSurgeon = false;
					for (DePuyEventSurgeonVO surg : vo.getSurgeonList()) {
						if (StringUtil.checkVal(surg.getSurgeonName()).equals(rs.getString("surgeon_nm")))
							hasSurgeon = true;
					}
					if (!hasSurgeon) {
					...
					if (roleId == MITEK_READ_ONLY_ROLE && profileId != null)
		...
		List<UserDataVO> profiles = new ArrayList<>();
		for (DePuyEventSeminarVO vo : data) {
			profiles.add(vo.getOwner());
		}

		try {
			ProfileManagerFactory.getInstance(getAttributes()).populateRecords(dbConn, profiles);
		} catch (DatabaseException|NullPointerException e) {
			//this is not fatal, so we're not going to throw it.  Only one data element will be missing from the page.
			log.error("could not load user profiles ", e);
		}
			...
			while (rs.next()) {
				if (vo == null) {
					...
					if (roleId == MITEK_READ_ONLY_ROLE)
						vo.setReadOnly(profileId != null && !profileId.equals(vo.getProfileId()));
				}
	 ...
	 * @throws ActionException
	 */
	protected void retrieveSurgeons(DePuyEventSeminarVO vo) {
	 ...
	 * @throws ActionException
	 */
	protected void retrieveConsignees(DePuyEventSeminarVO vo) {
		StringBuilder sql = new StringBuilder(100);
		sql.append("select * from ").append(attributes.get(Constants.CUSTOM_DB_SCHEMA));
	...
	private Collection<DePuyEventSeminarVO> loadOutstandingItems(ActionRequest req, 
		...
		try (PreparedStatement ps = dbConn.prepareStatement(sql.toString())) {
			...
			while (rs.next()) {
				...
				if (vo.getActionItems() != null && !vo.getActionItems().isEmpty())
					data.put(id, vo);
			}
		} catch (SQLException sqle) {
	...
	protected void loadSurveyReponses(List<DePuyEventSeminarVO> seminars, boolean isMitek) {
		...
		DBUtil.preparedStatmentQuestion(seminars.size(), sql);
