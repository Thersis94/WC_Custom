...
/****************************************************************************
 * <b>Title</b>: EventPostcardEmailer.java
 * <p/>
...
public class DePuyPostcardEmailerV2 extends AbstractPostcardEmailer {
	...
	/**
	 * sends the site admin an email whenever a user downloads patient
	 * data/reports
	 * 
	 * @param postcard
	 * @param user
	 ...
	 * @param site
	 */
	public void notifyAdminOfListPull(DePuyEventPostcardVO postcard,
			UserDataVO user, int type, SiteVO site) {
		List<DePuyEventEntryVO> events = postcard.getDePuyEvents();
		Iterator<DePuyEventEntryVO> iter = events.iterator();

		StringBuilder msg = new StringBuilder();
		msg.append(user.getFirstName()).append(" ")
				.append(user.getLastName());
		msg.append(" (").append(user.getEmailAddress())
				.append(") has pulled ");
		...
		while (iter.hasNext()) { // append events list to the message body
			...
			msg.append(event.getEventName()).append(" (")
					.append(event.getCityName());
		...
		try {
			...
			mail.setSubject(postcard.getProductName()
					+ " Community Education data download ("
					+ postcard.getRSVPCodes() + ")");
	...
	public void sendApprovedResponse(SMTServletRequest req) {
		SiteVO site = (SiteVO) req.getAttribute(Constants.SITE_DATA);
		UserDataVO user = (UserDataVO) req.getSession().getAttribute(
				Constants.USER_DATA);
		String rcptEmail = site.getAdminEmail();
		DePuyEventPostcardVO postcard = (DePuyEventPostcardVO) req
				.getAttribute("postcard");
		try {
	...
	/**
	 * send vendor and DePuy PMs an email with postcard info/layout and
	 * critical dates
	 * 
	 * @param req
	...
	public void sendVendorSummary(SMTServletRequest req) {
		...
		DePuyEventPostcardVO postcard = (DePuyEventPostcardVO) req
				.getAttribute("postcard");
		AbstractSBReportVO rpt = (AbstractSBReportVO) req
				.getAttribute(Constants.BINARY_DOCUMENT);
		...
		StringTokenizer st = new StringTokenizer(
				(String) getAttribute("eventAnnounceEmails"), ",");
		...
		try {
			...
			mail.setSubject("DePuy Community Education; New Postcard Announcement "
					+ postcard.getRSVPCodes());
			mail.setFrom(site.getMainEmail());
			mail.setTextBody(msg.toString());
			mail.addAttachment(rpt.getFileName(), rpt.generateReport()); // add
																// the
																// Excel
																// report
																// to
	...
	/**
	 * send DePuy TGMs and sales rep an email with postcard info and
	...
	public void sendPreAuthPaperwork(SMTServletRequest req) {
		...
		StringBuilder msg = new StringBuilder();
		msg.append("<p>").append(postcard.getProductName().toUpperCase())
				.append(" Seminar(s) ").append(postcard.getRSVPCodes());
		msg.append(" submitted by ").append(
				postcard.getOwner().getFirstName());
		...
		try {
			...
			EmailMessageVO mail = new EmailMessageVO();
			mail.addRecipients(emails);
			mail.setSubject("DePuy Synthes Joint Reconstruction Community Education; New Postcard Announcement "
					+ postcard.getRSVPCodes());
			mail.setFrom(site.getMainEmail());
	...
	public void sendPostcardCancellation(SMTServletRequest req) {
		...
		try {
			...
			EmailMessageVO mail = new EmailMessageVO();
			mail.addRecipients(emails);
			mail.setSubject("DePuy Community Education; Postcard Canceled "
					+ postcard.getRSVPCodes());
			mail.setFrom(site.getMainEmail());
			mail.setTextBody(msg.toString());
			mail.addAttachment(rpt.getFileName(), rpt.generateReport()); // add
	...
	public void sendSRCApprovalRequest(SMTServletRequest req) {
		...
		msg.append("Follow the link below to review and approve this request.");
