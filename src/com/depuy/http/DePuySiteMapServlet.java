...
public class DePuySiteMapServlet extends SiteMapServlet {
	...
	protected List<Node> loadCustomLowPriPages(SMTServletRequest req, SiteVO site, UserRoleVO role) {
		...
		if ("DPY_SYN".equals(site.getOrganizationId())) {
			pages = this.loadDSProducts(req, "DS_PRODUCTS", "/products/" + sc.getAttribute(Constants.QS_PATH));
			pages.addAll(this.loadDSProducts(req, "DS_PROCEDURES", "/procedures/" + sc.getAttribute(Constants.QS_PATH)));
		} else if ("DPY_SYN_EMEA".equals(site.getOrganizationId())) {
			pages = this.loadDSProducts(req, "DS_PRODUCTS_EMEA", "/products/" + sc.getAttribute(Constants.QS_PATH));
			pages.addAll(this.loadDSProducts(req, "DS_PROCEDURES_EMEA", "/procedures/" + sc.getAttribute(Constants.QS_PATH)));
		} else if ("DPY_SYN_INST".equals(site.getOrganizationId())) {
			pages = this.loadDSISolrAssets(site);
		} else {
	 ...
	 * @return
	 */
	protected List<Node> loadDSProducts(SMTServletRequest req, String catalogId, String sitePg) {
