...
/* Dave Bargerhuff - Used this source so that testing FTP transfers would
 * work with local dev server.
 * 
 * 01-05-2009: Added check for response starting with "230-".
 * 11-30-2010: Added code to compensate for FTP servers configured
 * with a greeting so that the greeting message is handled properly
 * during a user login.
 */
...
/* 
 ...
 $Author: beaker $
 $Id: SimpleFTP.java,v 1.7 2010/12/01 04:50:02 beaker Exp $
...
public class SimpleFTP {
  ...
  public synchronized void connect(String host, int port, String user,
    ...
    // interpret the response: If the FTP server is configured with a greeting message 
    // we need to cycle through it.
    response = readLine();
    while (response.startsWith("230-") || response.length() == 0) {
	    response = readLine();
	    if (DEBUG) System.out.println("response: " + response);
    }
    
    // check for a successful login.  throw exception on fail.
    if (! response.startsWith("230 ")) {
		throw new IOException("SimpleFTP was unable to log in with the supplied password: " + pass + "|" + response);
    }
