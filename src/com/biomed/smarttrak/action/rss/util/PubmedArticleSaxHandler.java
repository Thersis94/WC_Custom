...
public class PubmedArticleSaxHandler extends DefaultHandler {
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		...
		if(isDate) {
			data.put(type, val);
		} else if(!data.containsKey(SearchType.PMID) && SearchType.PMID.equals(type)) {
			data.put(type, buildValueString(type, val));
		} else if(!SearchType.DAY.equals(type) && !SearchType.MONTH.equals(type) && !SearchType.YEAR.equals(type) && !SearchType.PMID.equals(type)) {
			data.put(type, buildValueString(type, val));
		}
	}
