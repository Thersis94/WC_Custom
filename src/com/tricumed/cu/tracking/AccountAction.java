...
public class AccountAction extends SBActionAdapter {
	...
	private static final String CU_ACCT_TBL = "tricumed_cu_account ";
	...
	private Object deleteAccount(AccountVO avo, String customDb) {
		...
		sql.append("delete from ").append(customDb).append(CU_ACCT_TBL);
	...
	private Object updateAccount(AccountVO avo, String customDb) {
		...
		if (StringUtil.checkVal(avo.getAccountId()).length() == 0) {
			...
			sql.append("insert into ").append(customDb).append(CU_ACCT_TBL);
			...
			sql.append("update ").append(customDb).append(CU_ACCT_TBL);
