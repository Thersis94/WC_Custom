...
public abstract class AbstractTransAction extends SBActionAdapter {
	 ...
	 */
	protected List<UserDataVO> retrieveAdministrators(ActionRequest req) {
		SiteVO site = (SiteVO) req.getAttribute(Constants.SITE_DATA);
		UserAction ua = new UserAction(this.actionInit);
		ua.setAttributes(attributes);
		ua.setDBConnection(dbConn);
		return ua.loadUserList(SecurityController.ADMIN_ROLE_LEVEL, site.getOrganizationId());
	}
	 ...
	 * @throws ActionException
	 */
	protected List<UserDataVO> retrieveUsers(ActionRequest req, Integer roleLevel) {
		SiteVO site = (SiteVO) req.getAttribute(Constants.SITE_DATA);
		UserAction ua = new UserAction(this.actionInit);
		ua.setAttributes(attributes);
		ua.setDBConnection(dbConn);
		return ua.loadUserList(roleLevel, site.getOrganizationId());
	}
	...
	protected void insertTransaction( TransactionVO vo ) throws SQLException{
		...
		sql.append("insert into ").append(getAttribute(Constants.CUSTOM_DB_SCHEMA)).append("tricumed_cu_transaction ");
