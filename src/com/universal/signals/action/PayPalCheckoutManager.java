...
public class PayPalCheckoutManager {
	 ...
	 */
	public PayPalCheckoutManager(ActionRequest req, ShoppingCartVO cart) throws InvalidDataException {
	 ...
	 */
	protected void processTransaction() throws EncryptionException, IOException, SQLException {
	...
	private void checkCommonErrors(Map<String,String> rMap) {
		...
		if (errInfo != null && ! "success".equalsIgnoreCase(errInfo)) {
	 ...
	 * @throws EncryptionException 
	 */
	private MerchantInfoVO retrieveMerchantInfo() throws 	EncryptionException, SQLException {
	...
	private void init (String transaction, ShoppingCartVO cart) 
		...
		if ("set".equalsIgnoreCase(transaction)) {
			transactionType = TransactionType.EXPRESS_CHECKOUT_SET;
		} else if ("get".equalsIgnoreCase(transaction)) {
			transactionType = TransactionType.EXPRESS_CHECKOUT_GET;
		} else if ("do".equalsIgnoreCase(transaction)) {
	...
	private PaymentVO buildPayment() {
		return new PaymentVO(encryptionKey);
	}
