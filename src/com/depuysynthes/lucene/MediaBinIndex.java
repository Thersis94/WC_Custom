...
public class MediaBinIndex implements SMTCustomIndexIntfc {
	protected static Logger log = Logger.getLogger(MediaBinIndex.class);
	 ...
	 * 
	 * @see
	 * com.smt.sitebuilder.search.lucene.custom.SMTCustomIndexIntfc#addIndexItems
	 * (java.sql.Connection, com.siliconmtn.cms.CMSConnection,
	 * org.apache.lucene.index.IndexWriter)
	 */
	@Override
	public void addIndexItems(Connection conn, CMSConnection cmsConn,
			IndexWriter writer, Properties config) {
	 ...
	 * @param writer
	 */
	protected void indexFiles(List<MediaBinAssetVO> metaData,
			IndexWriter writer, String fileRepos) {
		...
		for (int i = 0; i < metaData.size(); i++) {
			...
			// ensure click-to URLs bounce through our redirector for version
			// control. leading slash added by WC's View.
			...
			try {
				...
				log.error("Unable to index asset "
						+ vo.getDpySynMediaBinId(), e);
			}
	...
	private byte[] loadFile(MediaBinAssetVO vo, String fileRepos) {
		byte[] data = null;
		try {
			String fileNm = StringUtil.replace(vo.getRevisionLvlTxt() + "/"
					+ vo.getAssetNm(), "/", File.separator);
			log.debug("loading file: " + fileRepos + fileNm);
			data = fileManager.retrieveFile(fileRepos + fileNm);
		} catch (Exception e) {
			log.error("could not load file for " + vo.getDpySynMediaBinId(),
					e);
		}
	...
	private List<MediaBinAssetVO> loadMetaData(Connection conn, String dbSchema) {
		...
		for (int x = MediaBinAdminAction.PDF_ASSETS.length; x > 0; x--)
			sql.append(",?");
		for (int y = MediaBinAdminAction.VIDEO_ASSETS.length; y > 0; y--)
			sql.append(",?");
		sql.append(")");
		log.debug(sql);
		...
		try {
			...
			for (String at : MediaBinAdminAction.PDF_ASSETS)
				ps.setString(++i, at);
			for (String vt : MediaBinAdminAction.VIDEO_ASSETS)
				ps.setString(++i, vt);
	...
	private String parseDownloadType(String downloadType, boolean isVideo) {
		...
		if (isVideo) {
			// this is a video, set the type as 'Video' or add 'Video' to the
			// existing type(s).
			if (tmp.length() == 0) {
				tmp = "Video";
			} else {
				if (!tmp.toLowerCase().contains("video"))
					tmp += ",Video";
			}
    ...
    /**
     * some constants for the business unit names.  The results coming back from 
     * Mediabin have no standardized notation.
     */
