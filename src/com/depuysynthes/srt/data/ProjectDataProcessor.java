...
public class ProjectDataProcessor extends FormDataProcessor {
	...
	public void saveProjectRecord(SRTProjectVO project) throws DatabaseException{
		...
		try {
			...
			//Get Co Project Id Generated by Trigger.
			populateCoProjectId(project);
	...
	/**
	 * Load Co Project Id into the given Project.
	 * @param project
	 */
	private void populateCoProjectId(SRTProjectVO project) {
		StringBuilder sql = new StringBuilder(150);
		sql.append(DBUtil.SELECT_CLAUSE).append(" CO_PROJECT_ID ");
		sql.append(DBUtil.FROM_CLAUSE).append((String)attributes.get(Constants.CUSTOM_DB_SCHEMA));
		sql.append("DPY_SYN_SRT_PROJECT ");
		...
		try(PreparedStatement ps = dbConn.prepareStatement(sql.toString())) {
			...
			if(rs.next()) {
				project.setCoProjectId(rs.getString("CO_PROJECT_ID"));
			}
		} catch (SQLException e) {
			log.error("Error Loading Co Project Id", e);
		}
	}
