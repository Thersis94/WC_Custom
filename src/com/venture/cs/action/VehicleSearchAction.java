...
public class VehicleSearchAction extends SBActionAdapter {
     ...
     */
	public void searchVehicles(SMTServletRequest req, String type)
			throws ActionException {
		...
		// if this is an archive search, return vin if search returned no
		// results, we will still
		// return a data size of 0.
		if (type.equals(SEARCH_ARCHIVE) && data.size() == 0) {
	...
	private String buildArchiveSearchString(String vin) {
		...
		sql.append("where a.ORGANIZATION_ID = ? and a.DEALER_TYPE_ID = ? ");
		// add vin if specified
		if (vin.length() > 0)	sql.append("AND b.VIN like ? ");
			...
			if (type.equals(SEARCH_ARCHIVE)) {
				...
				while (rs.next()) {
					...
					if (currId.equals(prevId)) {
						// if another record for same vehicle, just check
						// for action required
     ...
     * 
     */
	private String updateProfile(SMTServletRequest req) throws SQLException,
			ActionException, DatabaseException {
		 ...
		 * ) req.setParameter("state", req.getParameter("unitedStates")); else
		 * req.setParameter("state", req.getParameter("canada"));
		 */
		ProfileManager pm = ProfileManagerFactory.getInstance(attributes);
		UserDataVO user = new UserDataVO(req);

		if (profileId.length() == 0) profileId = pm.checkProfile(user, dbConn);
